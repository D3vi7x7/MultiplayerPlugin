...
AMultiplayerPluginCharacter::AMultiplayerPluginCharacter():
	CreateSessionCompleteDelegate(FOnCreateSessionCompleteDelegate::CreateUObject(this, &ThisClass::OnMakeSessionComplete)),
...
void AMultiplayerPluginCharacter::OnMakeSessionComplete(FName SessionName, bool bWasSuccessful)
{
	if (bWasSuccessful)
	{
		if (GEngine)
		...
		if (MatchType == "FreeForAll")
		{
			if (GEngine)
			{
				GEngine->AddOnScreenDebugMessage(
					...
					15.f,
					FColor::Green,
					FString::Printf(TEXT("Joining %s"), *MatchType)
				);
			}
	...
	if (OnlineSessionInterface->GetResolvedConnectString(NAME_GameSession, Address))
	{
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(
				...
				15.f,
				FColor::Green,
				FString::Printf(TEXT("Connecting to %s"), *Address)
			);
		}
